...
* **755 class dependencies** bypass ``analysis.spi`` and reach into ``analysis`` repositories or
  entities (rule 2). **Two cycles, 71 dependencies**: ``monitor ↔ insights`` (the decoupling
  event ``NodeRoleChangedEvent`` already exists, unevenly applied) and ``analysis ↔ file``
  (single edge — ``file`` needs only ``AsyncAnalysisService``). Rule 1 holds at **zero**
  violations.
