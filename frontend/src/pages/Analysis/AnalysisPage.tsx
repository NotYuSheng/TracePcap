...
export const AnalysisPage = () => {
  ...
  if (error) return <ErrorMessage title="Failed to Load Analysis" message={error.message} onRetry={refetch} />;
  if (!data) return <ErrorMessage title="No Data Available" message="No analysis data found for this file." />;
    ...
    <div className="analysis-page">
      <div className="analysis-header mb-4 d-flex justify-content-between align-items-start">
        ...
        <div className="d-flex flex-column align-items-end gap-1">
          ...
          {reportError && (
            <small className="text-danger">{reportError}</small>
          )}
        </div>
      </div>
      ...
      <ul className="nav nav-tabs" style={{ overflowX: 'auto', overflowY: 'hidden', flexWrap: 'nowrap', display: 'flex' }}>
        <li className="nav-item">
          <button style={{ whiteSpace: 'nowrap' }} className={`nav-link ${activeTab === 'overview' ? 'active' : ''}`} onClick={() => handleTabChange('overview')}>
            <i className="bi bi-speedometer2 me-2"></i>Overview
          </button>
        </li>
        <li className="nav-item">
          <button style={{ whiteSpace: 'nowrap' }} className={`nav-link ${activeTab === 'conversations' ? 'active' : ''}`} onClick={() => handleTabChange('conversations')}>
            <i className="bi bi-arrow-left-right me-2"></i>Conversations
          </button>
        </li>
        <li className="nav-item">
          <button style={{ whiteSpace: 'nowrap' }} className={`nav-link ${activeTab === 'story' ? 'active' : ''}`} onClick={() => handleTabChange('story')}>
            <i className="bi bi-journal-text me-2"></i>Story
          </button>
        </li>
        <li className="nav-item">
          <button style={{ whiteSpace: 'nowrap' }} className={`nav-link ${activeTab === 'filter-generator' ? 'active' : ''}`} onClick={() => handleTabChange('filter-generator')}>
            <i className="bi bi-funnel me-2"></i>Filter Generator
          </button>
        </li>
        <li className="nav-item">
          <button style={{ whiteSpace: 'nowrap' }} className={`nav-link ${activeTab === 'extracted-files' ? 'active' : ''}`} onClick={() => handleTabChange('extracted-files')}>
            <i className="bi bi-file-earmark-arrow-down me-2"></i>Extracted Files
          </button>
        </li>
        <li className="nav-item">
          <button style={{ whiteSpace: 'nowrap' }} className={`nav-link ${activeTab === 'network-diagram' ? 'active' : ''}`} onClick={() => handleTabChange('network-diagram')}>
