...
public class AnalysisController {
     ...
     * Returns:
     * - 200 OK: Analysis completed successfully (with data)
     * - 202 Accepted: Analysis still in progress (with Retry-After header)
    ...
    public ResponseEntity<AnalysisSummaryResponse> getAnalysisSummary(@PathVariable UUID fileId) {
        ...
        switch (analysis.getStatus()) {
            case PENDING:
            case IN_PROGRESS:
                log.info("Analysis for file {} is {}, returning 202 Accepted", fileId, analysis.getStatus());
                HttpHeaders headers = new HttpHeaders();
                headers.add("Retry-After", "2"); // Retry after 2 seconds
                return ResponseEntity.status(HttpStatus.ACCEPTED)
                        .headers(headers)
