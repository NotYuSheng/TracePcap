...
public class ConversationsController {
  ...
  public ResponseEntity<PagedResponse<ConversationResponse>> getConversations(
    ...
    ConversationFilterParams params =
        buildFilterParams(
            ip, protocols, apps, categories, hasRisks, fileTypes, sortBy, sortDir, search);

    log.info(
        "GET /api/conversations/{} - page:{}, pageSize:{}, ip:{}, protocols:{}, apps:{}, categories:{}, hasRisks:{}, fileTypes:{}, sortBy:{} {}",
        fileId,
        page,
        pageSize,
        params.getIp(),
  ...
  public void exportConversations(
    ...
    writer.println(
        "srcIp,srcPort,dstIp,dstPort,protocol,appName,category,hostname,packetCount,totalBytes,durationMs,startTime,endTime,flowRisks");
    for (ConversationResponse r : rows) {
      String flowRisksValue = r.getFlowRisks() != null ? String.join("; ", r.getFlowRisks()) : "";
      writer.printf(
          "%s,%s,%s,%s,%s,%s,%s,%s,%d,%d,%d,%s,%s,%s%n",
