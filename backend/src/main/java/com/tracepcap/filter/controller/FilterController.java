...
@Tag(
    ...
    description = "APIs for natural language to pcap filter generation and execution")
public class FilterController {
  ...
  public ResponseEntity<FilterGenerationResponse> generateFilter(
      @PathVariable String fileId, @Valid @RequestBody FilterGenerationRequest request) {

    log.info(
        "Generating filter for file {} with query: {}", fileId, request.getNaturalLanguageQuery());

    FilterGenerationResponse response =
        filterService.generateFilter(UUID.fromString(fileId), request.getNaturalLanguageQuery());
  ...
  @Operation(
      ...
      description =
          "Applies a BPF filter to a PCAP file and returns matching packets with pagination support")
  public ResponseEntity<FilterExecutionResponse> executeFilter(
    ...
    FilterExecutionResponse response =
        filterService.executeFilter(UUID.fromString(fileId), request.getFilter(), page, pageSize);
